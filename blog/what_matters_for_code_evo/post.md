...
This led me down a rabbit hole, comparing some simple baselines to much fancier code evolution methods and finding that the baselines often perform the same if not better. Trying to understand why the simple methods worked so well uncovered various insights and shortcomings with how code evolution is used.
...
For reinforcement learning there have been an abundance of papers over the years showing that methods are not evaluated properly, seemingly innocous design choices are actually significant, and some improvements are not as robust as they seemed [@agarwal2021deep;@henderson2018deep;@engstrom2020implementation;@huang202237]. Code evolution might be a similar wild west to what RL once was, where the field is blooming but what matters isn't yet fully understood.

More methodical approaches, like relying on simple methods and gradually building them up, could offer a way forward. When something seems fishy it can be investigated in isolation, and the sources of improvements are much more clear.^[{There are many interesting tidbits we found while investigating the baselines that didn't make it neither to the paper nor to this blogpost. For example, although many code evolution methods have various mechanisms designed to increase diversity, in practice they tend to get locked into bad search paths. This leads to "code bloat" [@langdon1997fitness], where the programs become longer and longer but effective improvements grind to a halt.}] This is only part of the story, as good comparisons also require standardized benchmarks, which are only starting to emerge.
...
You can find the full paper here(TODO) and the code here(TODO), including some minimal example notebooks. In conclusion, we recommend that:

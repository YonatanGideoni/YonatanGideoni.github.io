...
Denoting the $n\text{th}$ Hermite polynomial as $H_n$, Gonçalves et al. show that functions of the form $f(x)=\sum_{n=0}^\infty \alpha_nH_{4n}(\sqrt{2\pi}x)e^{-\pi x^2}$ fulfill the two conditions given that the coefficients $\alpha_n$ are chosen so that $f(0)=0$. As here $\hat{f}(x)=f(x)$, this automatically fulfills condition (b). As even Hermite functions are even, this fulfills condition (a).
